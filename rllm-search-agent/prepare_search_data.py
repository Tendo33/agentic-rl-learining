...
def process_wikipedia_corpus(save_path: str):
    print("Processing existing Wikipedia corpus file...")
    ...
    if not os.path.exists(wiki_gz_file):
        print(f"Error: wiki-18.jsonl.gz not found at {wiki_gz_file}")
        print("Please make sure you've downloaded the file to the correct location.")
        return None

    print(f"Found existing compressed corpus at {wiki_gz_file}")
    ...
    try:
        ...
        print(f"Error processing Wikipedia corpus: {e}")
        return None
...
def setup_search_data(data_dir: str = "./search_data", max_docs: int | None = None):
    ...
    wiki_file = process_wikipedia_corpus(data_dir)
