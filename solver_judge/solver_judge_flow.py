...
class Solver:
    ...
    async def generate_solutions(
        self, problem: str, n_solutions: int = 2
    ) -> list[Trajectory]:
        tasks = [
    ...
    def _parse_solver_response(self, response: str) -> str:
        answer_match = re.search(
            r"<answer>(.*?)</answer>", response, re.IGNORECASE | re.DOTALL
        )
        if answer_match:
...
class Judge:
    ...
    async def judge_solutions(self, problem: str, solutions: list[str]) -> Trajectory:
        messages = [
            {"role": "user", "content": self._create_judge_prompt(problem, solutions)}
        ]
        output: ModelOutput = await self.rollout_engine.get_model_response(messages)
        return Trajectory(
            name="judge",
...
class SolverJudgeWorkflow(Workflow):
    ...
    async def run(self, task: dict, uid: str, **kwargs) -> Episode:
        ...
        # Compute metrics
        solver_acc = sum(traj.steps[0].reward for traj in solver_trajectories) / len(
            solver_trajectories
        )
