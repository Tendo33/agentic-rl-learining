...
def prepare_countdown_data():
    ...
    if train_size < 2 * stage_size:
        print(
            f"Warning: Training set has only {train_size} examples, but need {2 * stage_size} for both stages"
        )
    ...
    train_dataset = DatasetRegistry.register_dataset(
        "countdown", train_dataset, "train"
    )
    test_dataset = DatasetRegistry.register_dataset("countdown", test_dataset, "test")
    stage2_dataset = DatasetRegistry.register_dataset(
        "countdown", stage2_dataset, "stage2_train"
    )
    stage3_dataset = DatasetRegistry.register_dataset(
        "countdown", stage3_dataset, "stage3_train"
    )
...
if __name__ == "__main__":
    train_dataset, test_dataset, stage2_dataset, stage3_dataset = (
